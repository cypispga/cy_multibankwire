...
class cy_multibankwire extends PaymentModule
{
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->l('Cy Multi Wire payment', array(), 'Modules.cy_multibankwire.Admin');
        $this->description = $this->l('Accept payments by bank transfer.', array(), 'Modules.cy_multibankwire.Admin');
        $this->confirmUninstall = $this->l('Are you sure about removing these details?', array(), 'Modules.cy_multibankwire.Admin');
        if (!isset($this->owner) || !isset($this->details) || !isset($this->address)) {
            $this->warning = $this->l('Account owner and account details must be configured before using this module.', array(), 'Modules.cy_multibankwire.Admin');
        }
        if (!count(Currency::checkPaymentCurrencies($this->id))) {
            $this->warning = $this->l('No currency has been set for this module.', array(), 'Modules.cy_multibankwire.Admin');
        }
            ...
            foreach ($languages as $lang) {
            	foreach ($curencies as $c) {
			            if (!Tools::getValue($c['iso_code'].'_CY_MILTI_BANK_WIRE_DETAILS_'.$lang['id_lang'])) {
			                $this->_postErrors[] = $this->l('Account details are required.', array(), 'Modules.cy_multibankwire.Admin')." [".$c['iso_code']."][".$lang['iso_code']."]";
			            } elseif (!Tools::getValue($c['iso_code'].'_CY_MILTI_BANK_WIRE_OWNER_'.$lang['id_lang'])) {
			                $this->_postErrors[] = $this->l('Account owner is required.', array(), "Modules.cy_multibankwire.Admin")." [".$c['iso_code']."][".$lang['iso_code']."]";
			            }
					}
			}
        }
    ...
    public function hookPaymentOptions($params)
    {
        ...
        $newOption->setModuleName($this->name)
                ->setCallToActionText($this->l('CY Pay by bank wire', array(), 'Modules.cy_multibankwire.Shop'))
    ...
    public function renderForm()
    {
    	...
    	foreach($curencies as $c){
				...
				$formfields[] =[
			            'type' => 'text',
			                'label' => $this->l('Account owner', array(), 'Modules.cy_multibankwire.Admin')." [".$c['iso_code']."]",
				...
				$formfields[] =[
			                'type' => 'textarea',
			                'label' => $this->l('Account details', array(), 'Modules.cy_multibankwire.Admin')." [".$c['iso_code']."]",
			                'name' => $c['iso_code'].'_CY_MILTI_BANK_WIRE_DETAILS',
			                'desc' => $this->l('Such as bank branch, IBAN number, BIC, etc.', array(), 'Modules.cy_multibankwire.Admin'),
			                'required' => true,
			                'lang' => true,
			                'tab' => $c['iso_code'],
			           ];
				$formfields[] =[
			                'type' => 'textarea',
			                'label' => $this->l('Bank address', array(), 'Modules.cy_multibankwire.Admin')." [".$c['iso_code']."]",
        ...
        $fields_form = array(
            'form' => array(
                'legend' => array(
                    'title' => $this->l('Account details', array(), 'Modules.cy_multibankwire.Admin'),
                    'icon' => 'icon-envelope'
                ),
                'input' => $formfields,
                'submit' => array(
        ...
        $fields_form_customization = array(
            'form' => array(
	           	'tabs'	=> $tabs,
                'legend' => array(
                    'title' => $this->l('Customization', array(), 'Modules.cy_multibankwire.Admin'),
                    'icon' => 'icon-cogs'
                ),
                'input' => array(
                    array(
                        'type' => 'text',
                        'label' => $this->l('Reservation period', array(), 'Modules.cy_multibankwire.Admin'),
                        'desc' => $this->l('Number of days the items remain reserved', array(), 'Modules.cy_multibankwire.Admin'),
                        'name' => 'CY_MILTI_BANK_WIRE_RESERVATION_DAYS',
                    ),
                    array(
                        'type' => 'textarea',
                        'label' => $this->l('Information to the customer', array(), 'Modules.cy_multibankwire.Admin'),
                        'name' => 'CY_MILTI_BANK_WIRE_CUSTOM_TEXT',
                        'desc' => $this->l('Information on the bank transfer (processing time, starting of the shipping...)', array(), 'Modules.cy_multibankwire.Admin'),
                        'lang' => true
                    ),
                    array(
                        'type' => 'switch',
                        'label' => $this->l('Display the invitation to pay in the order confirmation page', array(), 'Modules.cy_multibankwire.Admin'),
                        'name' => self::FLAG_DISPLAY_PAYMENT_INVITE,
                        'is_bool' => true,
                        'hint' => $this->l('Your country\'s legislation may require you to send the invitation to pay by email only. Disabling the option will hide the invitation on the confirmation page.', array(), 'Modules.cy_multibankwire.Admin'),
                        'values' => array(
                            array(
                                'id' => 'active_on',
                                'value' => true,
    ...
    public function getTemplateVarInfos()
    {
        ...
        $total = sprintf(
            $this->l('%1$s (tax incl.)', array(), 'Modules.cy_multibankwire.Shop'),
